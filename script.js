...
function setup() {
    let smallSide = Math.min(windowWidth, windowHeight * 0.85)
...
function setupGrid() {
    ...
    cells[endingCellX][endingCellY].color = color(200, 200, 200)
...
function draw() {
    if (gameOver) {
        ...
        cells.forEach(column => column.forEach(cell => {
            if (cell.traversed) {
                cell.draw(color(0, 0, 255))
            } else {
                cell.draw()
            }
        }))

        cells[activeCellX][activeCellY].draw(color(255, 0, 0))
